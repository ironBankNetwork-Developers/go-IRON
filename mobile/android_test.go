...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", iron.LightScryptN, iron.LightScryptP);
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "iron-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/iron/gmctest", "libs"} {
	...
	build.CopyFile(filepath.Join("libs", "iron.aar"), "iron.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "iron", "gmctest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
