Name "iron ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install iron binary
Section "iron" GMC_IDX
  SetOutPath $INSTDIR
  file {{.iron}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\iron.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\iron.exe" "attach" "" ""
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "" ""

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "iron incoming peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "iron outgoing peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "iron UDP discovery (UDP:30303)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "iron incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\iron.exe" "" "" "Ethereum" 30303 "" "" ""
  SimpleFC::AdvAddRule "iron outgoing peers (TCP:30303)" ""  6 2 1 2147483647 1 "$INSTDIR\iron.exe" "" "" "Ethereum" "" 30303 "" ""
  SimpleFC::AdvAddRule "iron UDP discovery (UDP:30303)" "" 17 2 1 2147483647 1 "$INSTDIR\iron.exe" "" "" "Ethereum" "" 30303 "" ""

  # Set default IPC endpoint (https://github.com/ethereum/EIPs/issues/147)
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "R" "HKLM" "\\.\pipe\iron.ipc"
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "A" "HKLM" "\\.\pipe\iron.ipc"
