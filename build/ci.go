...
var (
	// Files that end up in the iron*.zip archive.
	ironArchiveFiles = []string{
		"COPYING",
		executablePath("iron"),
	}

	// Files that end up in the iron-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archives (usually "ironstore/builds")`)
		ext    string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		...
		iron     = "iron-" + base + ext
		alltools = "iron-alltools-" + base + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(iron, ironArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{iron, alltools} {
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Where to upload the source package (usually "ppa:iron/iron")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "iron-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "iron.exe" {
			ironTool = file
		} else {
	...
	// first section contains the iron binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"iron":      ironTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.iron.nsi", filepath.Join(*workdir, "iron.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("iron-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "iron.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archive (usually "ironstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	build.MustRun(gomobileTool("bind", "--target", "android", "--javapkg", "org.iron", "-v", "github.com/iron-bank/go-IRON/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("iron.aar", filepath.Join(GOBIN, "iron.aar"))
		return
	}
	...
	archive := "iron-" + archiveBasename("android", env) + ".aar"
	os.Rename("iron.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	archive := "iron-" + archiveBasename("ios", env)
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "iron.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "iron.podspec", "--allow-warnings", "--verbose")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "ironstore/builds")`)
