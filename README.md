...
## First Steps
Before accessing the Dapp, you need to set the start the iron protocol, in order to get access to the blockchain and interact with the functionalities. 
...
### Full node on the iron test network

Transitioning towards developers, if you'd like to play around with creating iron contracts, you
almost certainly would like to do that without any real money involved until you get the hang of the
entire system. In other words, instead of attaching to the main network, you want to join the **test**
network with your node, which is fully equivalent to the main network, but with play-Ether only.

```
$ iron --testnet --fast --cache=512 console
```
...
Specifying the `--testnet` flag however will reconfigure your iron instance a bit:

 * Instead of using the default data directory (`~/.iron` on Linux for example), iron will nest
   itself one level deeper into a `testnet` subfolder (`~/.iron/testnet` on Linux). Note, on OSX
   and Linux this also means that attaching to a running testnet node requires the use of a custom
   endpoint since `iron attach` will try to attach to a production node endpoint by default. E.g.
   `iron attach <datadir>/testnet/iron.ipc`. Windows users are not affected by this.
 * Instead of connecting the main Iron Bank Network, the client will connect to the test network,
   which uses different P2P bootnodes, different network IDs and genesis states.

*Note: Although there are some internal protective measures to prevent transactions from crossing
over between the main network and test network, you should make sure to always use separate accounts
for play-money and real-money. Unless you manually move accounts, iron will by default correctly
...
With the genesis state defined in the above JSON file, you'll need to initialize **every** iron node
with it prior to starting it up to ensure all blockchain parameters are correctly set:

```
$ iron init path/to/genesis.json
```
...
```
$ iron --datadir=path/to/custom/data/folder --bootnodes=<bootnode-enode-url-from-above>
```
...
Mining on the public Iron Bank Network is a complex task as it's only feasible using GPUs, requiring
...
```
$ iron <usual-flags> --mine --minerthreads=1 --etherbase=0x0000000000000000000000000000000000000000
```
